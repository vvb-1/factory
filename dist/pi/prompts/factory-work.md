...
3. Spawn a subagent (general-purpose, run in background, **`model: opus`** — implementation quality is the product; the orchestration itself stays on the cheaper session model) with a **self-contained prompt** containing the full ticket (ID, description, all five sections), the worktree path and its ports/database, and these standing orders:
   ...
   - On success: push the branch, `gh pr create --title "..." --body "Fixes <ISSUE-ID>"`, move the ticket to `In Review` + `ai:needs-review` (remove `ai:in-progress`), post the **Handoff comment** in the exact format `/factory-ticket` step 6 defines (PR link, verification command + result, UX critique verdict, files vs `Owned Paths`, risks), and report terminal state `STATE: PR_OPEN`. For user-facing changes, attach before/after screenshots (reuse the critique round's captures) to the Linear ticket via the attachment upload and add "Screenshots on <ISSUE-ID>" to the PR body — do not commit screenshots into the repo.
