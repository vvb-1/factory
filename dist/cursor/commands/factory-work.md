...
3. Spawn a subagent (general-purpose, run in background, **`model: opus`** — implementation quality is the product; the orchestration itself stays on the cheaper session model) with a **self-contained prompt** containing the full ticket (ID, description, all five sections), the worktree path and its ports/database, and these standing orders:
   ...
   - Run the UX critique round after verification and before opening the PR when the change introduces or materially changes a user-completable flow, interaction, state transition, error/recovery path, responsive layout, authentication, payment, onboarding, or destructive action. Spawn `ux-critic`, fix in-scope `FIX-FIRST` findings, max 2 rounds, and file follow-ups to `Triage`. Skip it for isolated styling, copy-only edits, static content, icons/assets, and internal/admin-only surfaces unless the ticket identifies UX risk. In every PR, state `UX critique: required` or `UX critique: skipped — <reason>`.
   - On success: push the branch, `gh pr create --title "..." --body "Fixes <ISSUE-ID>"`, move the ticket to `In Review` + `ai:needs-review` (remove `ai:in-progress`), and post the **Handoff comment** in the exact format `/factory-ticket` step 6 defines (PR link, verification command + result, UX critique verdict, files vs `Owned Paths`, risks) — it is the merge stage's input, not a courtesy note. For user-facing changes, attach before/after screenshots (reuse the critique round's captures) to the Linear ticket via the attachment upload and add "Screenshots on <ISSUE-ID>" to the PR body — do not commit screenshots into the repo.
