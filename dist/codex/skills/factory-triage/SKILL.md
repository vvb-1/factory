...
2. **Specify** — investigate the codebase enough to write the full §5 AI-ready template: `Problem & Context`, observable `Acceptance Criteria`, `Source File Pointers` (verify the files actually exist **on `origin/<base>`**, not merely in your checkout — a pointer to a path that moved last week sends the implementation agent to a file that isn't there), `Owned Paths` (tight globs — these gate concurrent dispatch), and a `Verification Command` that actually runs in this repo. For non-code work, the evidence line replaces the verification command.
