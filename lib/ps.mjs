...
export function formatFactoryPsReport(snapshot, { colors = true, all = false, repo = null } = {}) {
  ...
  if (snapshot.controlPlane.length === 0) {
    ...
    for (const cp of snapshot.controlPlane) {
      ...
      if (cp.service === "serve") {
        status = apiProbe?.online ? col.green("online") : col.yellow("starting/busy");
      }
  ...
  const filteredWorktrees = all
    ? snapshot.worktrees
    : snapshot.worktrees.filter((w) => w.active || snapshot.devServers.some((d) => d.worktree === w.path));
  ...
  if (filteredWorktrees.length === 0 && snapshot.devServers.length === 0) {
    ...
    for (const wt of filteredWorktrees) {
      ...
      const pidsStr = Object.entries(wt.pids)
        .map(([k, v]) => `${k}:${v}`)
        .join(", ") || "-";
      const boundPorts = snapshot.ports
        .filter((lp) => Object.values(wt.pids).includes(lp.pid))
        .map((lp) => lp.port)
        .join(", ");
